...
import("@dimforge/rapier2d").then((RAPIER) => {
  ...
  const resizeCallback = () => {
    [...document.getElementsByTagName("canvas")].forEach((c) => {
      Object.assign(c.style, {
        ...((window.innerWidth * 777) / 1366 > window.innerHeight
  ...
  function loadFromStorage() {
    ...
    if (state) {
      ...
      CURRENT_MODE =
        state.mode || (state.minimizeDuplicates ? "casual" : "ranked");
  ...
  score.onclick = () => {
    if (
      confirm(
        "Are you sure you want to restart? Manually restarting means your score won't have a chance to go on the leaderboard"
      )
    ) {
  ...
  const LEADERBOARD_URL =
    "https://script.google.com/macros/s/AKfycbw6iTqt_fyO5OtTZ9de3pZUEglgvTH9tlVxkiPmlpkjaRpoqz0vn8IK_CddqT3F3OLsTw/exec";
  ...
  function updateLeaderboardStrings() {
    leaderboardModeToggle.innerText = leaderboardCasFocused
      ? "Ranked"
      : "Casual";
    leaderboardHeader.innerText = leaderboardCasFocused
      ? "Casual Leaderboard"
      : "Ranked Leaderboard";
    leaderboardSubtitle.innerText = leaderboardCasFocused
      ? "Balls drop in a random order in casual mode."
      : "Balls always drop in the same order in ranked mode.";
  }
  ...
  async function sendLeaderboardScore(scoreToSend, dataURL) {
    console.log(dataURL);
    let name = prompt(
      "Enter your name if you would like to submit your score to leaderboard. Use your real name and don't put anything bad pls 🙏"
    );
    ...
    if (name) {
      ...
      let response = await fetch(
        `https://www.purgomalum.com/service/json?text=${name}`
      );
    ...
    let newData = await fetch(
      `${LEADERBOARD_URL}?data=${encodeURIComponent(encryptedData)}`
    );
    ...
    let leaderboardImages =
      leaderboard.getElementsByClassName("leaderboardImage");
  ...
  function getLeaderboard() {
    if (
      (leaderboardCasFocused && CASUAL_LEADERBOARD.length > 0) ||
      (!leaderboardCasFocused && RANKED_LEADERBOARD.length > 0)
    ) {
      renderLeaderboard(
        leaderboardCasFocused ? CASUAL_LEADERBOARD : RANKED_LEADERBOARD
      );
      return;
    }
    let leaderboard = document.getElementById("leaderboardEntries");
    ...
    fetch(
      LEADERBOARD_URL + `?mode=${leaderboardCasFocused ? "casual" : "ranked"}`
    )
...
">
  ...
  elementButtons.forEach((button) => {
    ...
    button.onclick = () => {
      const updatedButtonList = eltm.handlePowerClick(
      ...
      updatedButtonList.forEach((afterButton) => {
        const buttonElement = elementButtons.find(
          (b) => b.getAttribute("element") === afterButton.name
        );
